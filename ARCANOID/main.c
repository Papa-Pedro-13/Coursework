...
void BallMove()
{
    ...
    if (gamestarted == 1) {
        ...
        // ∆Ë‚˚ ÎË ÍËÔË˜Ë?
        //каждый кадр проверяем остались ли блоки, проходя по всему полю
        //можно ввести переменную количества блоков, которая будет уменьшаться при уничтожении блока, и проверять ее значение вместо двух форов
        brick_health[0][0] = 0;
        int temp = 0;
        for (int i = 1; i <= rows; i++) {
            for (int j = 1; j <= col; j++) {
        ...
        // ∆Ë‚˚ ÎË ¯‡ËÍË?
        //проверяем остались ли мячики, можно опять же ввести переменную счетчик и избавиться от фора
        //также два ифа одинаковые и сводятся в один (вообще все такие ифы заменить на гоуту)
        for (int i = 0; i < max_balls; i++) {
            if (balls[i] == 1) {
                temp = 1;
                break;
    ...
    //оптимизацию не придумал не придумал
    ...
    if (gamemenu == 1) {
        ...
        //в меню включить какой-то из четырех уровней
        else {
        ...
        for (int v = 0; v < max_balls; v++) {
            ...
            //переменные для возможного бонуса
            for (int i = 1; i <= rows; i++) {
                for (int k = 1; k <= col; k++) {
                    ...
                    // ÕËÊÌˇˇ „‡Ì¸
                    //пока не стал разбираться с четырьмя длинными ифами, но повсей видимости это проверки на то
                    //в какую грань и с какой стороны прилетел мяч (и прилетел ли он в этот блок вообще)
                    if (((y_lu - 3 <= brick_pos_y[i][k] + brick_h && y_ld + 3 > brick_pos_y[i][k] + brick_h && (ball_x - ball_r <= brick_pos_x[i][k] + brick_w || ball_x + ball_r >= brick_pos_x[i][k])) || (y_ru - 3 <= brick_pos_y[i][k] + brick_h && y_rd + 3 > brick_pos_y[i][k] + brick_h && (ball_x - ball_r - 3 <= brick_pos_x[i][k] + brick_w || ball_x + ball_r + 3 >= brick_pos_x[i][k]))) && (ball_x - ball_r - 3 >= brick_pos_x[i][k] || ball_x + ball_r + 3 <= brick_pos_x[i][k] + brick_w) && (y_vel < 0) && (brick_status[i + 1][k] == 0)) { // ÕËÊÌˇˇ „‡Ì¸
                        //два одинаковых выражения в ифах, можно слить в один
                ...
                if (temp == 1) {
                    ...
                    rememberHittedRow = i;
                    //выбор бонуса, можно оптимизировать
                    if (isHit == 1) {
